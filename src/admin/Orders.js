...
const Orders = () => {
  ...
  const showOrdersLength = orders => {
    if(orders.length > 0){
      return (
        <h1 className="text-danger">Total number of orders: {orders.length}</h1>
      )
    } else {
      return <h1 className="text-danger">There are no orders.</h1>
    }
  }
  ...
  return (
    <Layout title="Orders" description={'Manage your orders'} className="container">
      <div className="row">
            ...
            return (
              <div className="mt-5" key={oIndex} style={{borderBottom: "5px solid indigo"}}>
                ...
                <ul className="list-group mb-2">
                  ...
                  <li className="list-group-item">Amount: {o.amount} HUF</li>
                  <li className="list-group-item">User: {o.user.name}</li>
                  <li className="list-group-item">Ordered placed: {moment(o.createdAt).fromNow()}</li>
